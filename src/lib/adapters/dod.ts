...
// ═══════════════════════════════════════════════════════════════════
// DOD.com.tr Adapter — Real scraping with graceful fallback
// ═══════════════════════════════════════════════════════════════════
...
export class DodAdapter extends BaseAdapter {
  ...
  async getDetail(listingId: string): Promise<ListingRaw | null> {
    try {
      ...
      $('.spec-list li, .detail-features li, .vehicle-specs li, .fleet-specs li, .specs-table tr').each((_, el) => {
      ...
      // Description
      const description = $('.description, .vehicle-description, .detail-description, #description, .fleet-description').text().trim();

      // Images
      const imageUrls: string[] = [];
      $('.gallery img, .vehicle-photos img, .detail-images img, .carousel img, .fleet-photos img').each((_, el) => {
      ...
      const listing: ListingRaw = {
        ...
        sellerType: 'Galeri', // DOD is a rental fleet sales platform
  ...
  /**
   * Build the DOD search URL from filter parameters.
   * Maps our internal filter names to DOD's query parameter names.
   */
  private buildSearchUrl(filters: SearchFilters): string {
    ...
    // Fuel type — DOD uses "yakitTipi"
    if (filters.fuelType) {
      params.set('yakitTipi', this.slugify(filters.fuelType));
    }

    // Transmission — DOD uses "vites"
    if (filters.transmission) {
      params.set('vites', this.slugify(filters.transmission));
    }
  ...
  private extractListingFromCard(
    ...
    const title =
      linkEl.attr('title') ||
      linkEl.text().trim() ||
      $el.find('.title, .card-title, .vehicle-title, .fleet-title, h2, h3').first().text().trim();

    // ── Price ────────────────────────────────────────────────────
    const priceText =
      $el.find('.price, .vehicle-price, .card-price, .fleet-price, .listing-price').first().text().trim();
    ...
    // ── City / Location ──────────────────────────────────────────
    const cityText =
      $el.find('.city, .location, .card-city, .fleet-location, [data-city]').first().text().trim();
    ...
    // DOD is a rental fleet sales platform — always Galeri
    const sellerType = 'Galeri';
  ...
  /**
   * Extract the listing ID from a DOD URL.
   * DOD URLs may contain a numeric ID segment at the end.
   */
  ...
  /**
   * Slugify a string for use in DOD URL paths.
